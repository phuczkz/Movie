...
import { BookOpen, List, Info, AlertCircle, Heart } from "lucide-react";
      ...
      <div className="relative rounded-3xl overflow-hidden bg-slate-900 border border-slate-800 shadow-2xl p-6 md:p-10 mb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 space-y-4 text-center md:text-left">
            ...
            <div className="pt-6">
              <h3 className="flex items-center gap-2 font-semibold text-slate-100 text-lg mb-2 justify-center md:justify-start">
            ...
            {chapters.length > 0 && (
              <div className="pt-4 flex flex-col md:flex-row gap-3">
                <Link
                  ...
                  className="inline-flex items-center justify-center gap-2 w-full md:w-auto px-8 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-purple-900/50 hover:scale-105"
                >
                ...
                <button
                  ...
                  disabled={favLoading}
                  className={`inline-flex items-center justify-center gap-2 w-full md:w-auto px-8 py-3 rounded-xl font-bold transition-all border shadow-lg hover:scale-105 ${isSaved
                      ? "bg-red-500/10 border-red-500 text-red-500"
                      : "bg-white/5 border-white/20 text-white hover:bg-white/10"
                    } ${favLoading ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  <Heart className={`w-5 h-5 ${isSaved ? "fill-red-500" : ""}`} />
                  {isSaved ? "Hủy Yêu Thích" : "Yêu Thích"}
                </button>
              </div>
            )}
          </div>
