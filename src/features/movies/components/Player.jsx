...
import { createHlsCustomType } from "./Player/player-hls-adapter.js";
...
const Player = ({
  ...
  useEffect(() => {
    ...
    // ── Build HLS customType (delegated to adapter module) ──
    let customType;
    if (isHls && Hls && Hls.isSupported()) {
      customType = createHlsCustomType({
        Hls,
        hlsConfigRef,
        hlsInstanceRef,
        pendingSeekRef,
    ...
    try {
      ...
      // Lưu posterUrl trên instance để adapter có thể truy cập khi cần switchUrl trong fatal error recovery
      artInstanceRef.current._hlsPosterUrl = posterUrl || "";
    } catch (err) {
    ...
    return () => {
      mountedRef.current = false;
      // Clear A/V desync watchdog & post-seek handlers
      ...
      if (videoEl?._seekCleanup) {
        videoEl._seekCleanup();
        videoEl._seekCleanup = null;
      }
