...
const ActorSection = memo(
  ({ actorsWithImages, isMobile = false, variant = "default" }) => {
    if (!actorsWithImages || actorsWithImages.length === 0) {
      if (isMobile)
        return (
          <p className="text-slate-400 p-4">Chưa có danh sách diễn viên.</p>
        );
      return null;
    }
      ...
      return (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <p className="text-xs uppercase tracking-[0.14em] text-slate-300 font-bold">
              Diễn viên
            </p>
            <span className="text-[10px] font-bold text-slate-500 bg-white/5 px-1.5 py-0.5 rounded-full">
    ...
    if (variant === "sidebar") {
      return (
        <div className="flex flex-col h-[280px] overflow-hidden group/actor-section">
          <div className="p-5 pb-2 flex items-center justify-between shrink-0">
            <p className="text-sm uppercase tracking-[0.14em] text-slate-300 font-bold">
              Diễn viên
            </p>
