...
// Module-level cache để lưu URL ảnh phim Trung Quốc đã tải thành công trong phiên làm việc
const loadedChinaCache = new Set();

/**
 * ChinaShowcase — Showcase phim Trung Quốc
 * Thiết kế card dạng landscape (16:9) tương tự TheaterShowcase
 * với backdrop images, tiêu đề, tập mới nhất/chất lượng, thể loại và nút play.
 */
const ChinaCard = ({ movie, priority = false }) => {
  ...
  const isCached = Boolean(thumbSrc && loadedChinaCache.has(thumbSrc));
  const isImageReady = loaded || isCached;

  useEffect(() => {
    if (imgRef.current?.complete && imgRef.current?.naturalWidth > 0) {
      if (thumbSrc) loadedChinaCache.add(thumbSrc);
  ...
  return (
    ...
    >
      <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-slate-800">
        ...
        {/* Quality and Episode badge - top right */}
        <div className="absolute top-3 right-3 z-10 flex items-center gap-1.5">
          {movie.episode_current && (
            <span className="inline-block rounded bg-rose-600/90 px-2 py-0.5 text-[11px] font-bold text-white uppercase backdrop-blur-sm shadow-sm">
              {movie.episode_current}
            </span>
          )}
...
const MemoChinaCard = memo(ChinaCard);
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="relative group/china">
        ...
        >
          {movies.map((movie, i) => (
            <MemoChinaCard key={movie.slug} movie={movie} priority={i < 2} />
          ))}
        </div>
        ...
        {canScrollLeft && (
          <button
            type="button"
            onClick={() => scroll("left")}
            className="hidden lg:flex absolute left-2 top-1/2 -translate-y-1/2 z-20 items-center justify-center w-10 h-10 rounded-full bg-black/60 border border-white/10 text-white backdrop-blur-sm opacity-0 group-hover/china:opacity-100 transition-opacity hover:bg-black/80 cursor-pointer"
        ...
        {canScrollRight && (
          <button
            ...
            className="hidden lg:flex absolute right-2 top-1/2 -translate-y-1/2 z-20 items-center justify-center w-10 h-10 rounded-full bg-black/60 border border-white/10 text-white backdrop-blur-sm opacity-0 group-hover/china:opacity-100 transition-opacity hover:bg-black/80 cursor-pointer"
