...
function WatchHistoryCard({ item, handleDelete, user }) {
  ...
  return (
    <div
      className="group relative flex flex-row sm:flex-col overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 bg-slate-800/50 sm:bg-slate-900 h-28 sm:h-auto sm:aspect-[3/4] transition-all hover:scale-[1.01] hover:border-emerald-500/50 hover:shadow-lg sm:hover:shadow-2xl"
    >
      {/* Image Section: Left side on Mobile, Background on Desktop */}
      <div className="w-20 min-w-[5rem] sm:w-full sm:absolute sm:inset-0 sm:z-0 flex-shrink-0 relative overflow-hidden bg-slate-900">
        {posterUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center p-2 text-center opacity-30 bg-slate-800">
            <span className="font-bold text-sm sm:text-xl text-slate-300 uppercase tracking-widest break-words overflow-hidden line-clamp-2">{movieName}</span>
          </div>
        )}
        {/* Gradient Overlay applies only on desktop */}
      ...
      <div className="absolute top-2 right-2 sm:top-3 sm:right-3 z-20">
        <button
          ...
          className="p-1.5 sm:p-2 text-slate-400 sm:text-white/70 hover:text-rose-400 sm:hover:text-white bg-slate-900/40 sm:bg-black/40 sm:hover:bg-rose-600/90 rounded-full backdrop-blur-md transition-all shadow-md active:scale-95"
      ...
      {/* Content Section: Right side on Mobile, Bottom on Desktop */}
      <div className="relative z-10 flex flex-1 flex-col p-3 sm:p-5 justify-center sm:justify-end overflow-hidden">
        <h3 className="mb-0.5 sm:mb-1 text-sm sm:text-lg font-bold text-white tracking-tight sm:drop-shadow-lg line-clamp-1 sm:line-clamp-2 pr-6 sm:pr-0">
          {movieName}
        </h3>
        
        <p className="text-[11px] sm:text-sm font-medium text-emerald-400 sm:text-emerald-300 sm:drop-shadow-md mb-0.5">
          {epLabel}
        </p>
        
        <div className="mt-2 sm:mt-0 flex items-center justify-between">
          <p className="text-[10px] sm:text-xs text-slate-400 sm:text-slate-300/90 sm:drop-shadow-md sm:mb-4">
